...
export function setContrastColor(color) {
  return getContrast(color, "black") > getContrast(color, "white")
    ? "black"
    : "white";
}
...
export function on3DModelLoaded(entity, callback) {
  if (!entity || !entity.components["gltf-model"]) {
    throw new Error(
      "3D model is not defined or entity does not have a 3D model component"
    );
  }
...
export function computeBbox(entity) {
  ...
  if (!entity) {
    console.warn(
      "Entity is undefined, return empty bounding box; the component that uses this bounding box may not work properly"
    );
    return defaultBbox;
  }
  ...
  if (!mesh) {
    console.warn(
      `Mesh of ${entity.tagName} is undefined, return empty bounding box; the component that uses this bounding box may not work properly`
    );
    return defaultBbox;
  }
 ...
 * @param {AEntity} el - The element to append.
 * @param {AEntity} parent - The parent element to append to.
 */
export function appendTo(el, parent = null) {
