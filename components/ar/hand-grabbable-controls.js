...
AFRAME.registerComponent("hand-grabbable-controls", {
  ...
  transferEntityOwnership: function () {
    ...
    var grabbingEls = this.el.sceneEl.querySelectorAll(
      "[hand-grabbable-controls]"
    );
    for (var i = 0; i < grabbingEls.length; ++i) {
      grabbingElComponent =
        grabbingEls[i].components["hand-grabbable-controls"];
  ...
  releaseGrabbedEntity: function () {
    ...
    grabbedEl.object3D.matrixWorld.decompose(
      this.auxVector,
      this.auxQuaternion,
      this.auxVector2
    );
  ...
  grab: function () {
    ...
    grabedObjectWorldPosition = grabbedEl.object3D.getWorldPosition(
      this.grabbedObjectPosition
    );

    this.grabDeltaPosition
      .copy(grabedObjectWorldPosition)
      .sub(this.pinchPosition);
    this.grabInitialRotation.copy(
      this.auxQuaternion.copy(this.wristRotation).invert()
    );

    this.originalUpdateMatrixWorld = grabbedEl.object3D.updateMatrixWorld;
    grabbedEl.object3D.updateMatrixWorld = function () {
      /* no op */
    };
  ...
  tock: function () {
    ...
    // 2.
    auxMatrix.identity();
    auxMatrix.makeRotationFromQuaternion(
      auxQuaternion.copy(this.wristRotation).multiply(this.grabInitialRotation)
    );
    grabbedObject3D.matrixWorld.multiply(auxMatrix);
    ...
    // 5.
    auxMatrix.makeScale(
      grabbedEl.object3D.scale.x,
      grabbedEl.object3D.scale.y,
      grabbedEl.object3D.scale.z
    );
    grabbedObject3D.matrixWorld.multiply(auxMatrix);
