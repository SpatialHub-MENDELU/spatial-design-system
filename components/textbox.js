...
AFRAME.registerComponent('textbox', {
    schema: {
      ...
      font: { type: 'string', default: 'https://raw.githubusercontent.com/etiennepinchon/aframe-fonts/master/fonts/robotomono/RobotoMono-Regular.json' },
    ...
    init() {
      ...
      this.el.setAttribute('virtualKeyboard', 'variant', this.el.getAttribute('variant'));	
  ...
  createTextbox() {
    ...
    const borderColor = this.el.getAttribute("variant") === "dark" ? VARIANT_LIGHT_COLOR : VARIANT_DARK_COLOR

    const textboxMesh = this.createMesh(width, height, this.el.getAttribute("variant") ==="dark" ? VARIANT_DARK_COLOR : VARIANT_LIGHT_COLOR, false);
    this.el.setObject3D('mesh', textboxMesh);

    const borderWidth = 0.008;
    const borderMesh = this.createMesh(width + 2 * borderWidth, height + 2 * borderWidth, borderColor, true);
    borderMesh.visible = this.data.isactivated

    this.el.setObject3D('border', borderMesh);
    
    // Creating the shadowBox mesh
    const shadowMesh = this.createMesh(width, height, this.el.getAttribute("variant") === 'dark' ? VARIANT_DARK_COLOR : VARIANT_LIGHT_COLOR, false, 0.5)
  ...
  setText() {
    ...
    clone.style.fontSize = `${this.el.getAttribute('sizeCoef') * (2/3) * 16}px`; 
    clone.textContent = 'a';
  ...
  switchActivated() {
    ...
    if(isActivated)
      this.el.startBlinking()
    else
      this.el.stopBlinking()

    // Switching the visibility of the border
    this.el.getObject3D('border').visible = isActivated;
    // Sending emit for virtualKeyboard component to display the keyboard
    this.el.emit('is-textbox-activated', {isActivated: this.data.isactivated, id: this.el.getAttribute('id')});
  },
  ...
  moveTextLine() {
    ...
    if (this.data.moveCursorByAmount + this.data.numberOfCharactersInTheBox <= 0 && this.data.numberOfCharactersInTheBox !== inputEl.value.length && this.el.components.virtualKeyboard.data.lastPressedKey === '<') {
    ...
    const maxCharsAmount = Math.floor(maxAllowedTextWidth/this.data.fontWidth) // Maximum number of chars that can fit in the textbox
    this.data.maxCharsAmount = maxCharsAmount
    const textElContentWidth = textEl.value.length * (textEl.width / textEl.wrapCount) // The actual width of the text content
    ...
    if(textElContentWidth > maxAllowedTextWidth || this.data.moveWholeTextInBoxByAmount <= 0) {
      this.el.setAttribute('text', {
        // Cutting the text to the last max characters, that can be displayed in the textbox
        value: inputEl.value.substring(inputEl.value.length - maxCharsAmount + this.data.moveWholeTextInBoxByAmount, inputEl.value.length + this.data.moveWholeTextInBoxByAmount),
      })
    }
  ...
  setEvents() {
    ...
    const setText = (lastKeyboardCharacter) => { 
      ...
      const lastPressedKey = this.el.components.virtualKeyboard.data.lastPressedKey || ''

      if (lastPressedKey !== ''){
        // Deleting a character
        if (lastPressedKey === 'DEL'){
          if(cursorIndex === 0) return // Do nothing if the cursor is at the beginning
          newValue = inputEl.value.slice(0, cursorIndex -1) + inputEl.value.slice(cursorIndex, inputEl.value.length)
          inputEl.value = newValue
        } else {
          // Inserting a character
          newValue = inputEl.value.substring(0, cursorIndex) + lastPressedKey + inputEl.value.substring(cursorIndex, inputEl.value.length);
          // Edge case when the cursor is at the beginning of the textbox and the text is longer than the textbox
          if(cursorIndex === 0 && inputEl.value.length >= this.data.maxCharsAmount) this.data.moveWholeTextInBoxByAmount -= 1
      ...
      const textColor = this.el.getAttribute('variant') === 'dark' ? "white" : "black"
      this.el.setAttribute('text',{
        align: 'left',
        value: newValue,
        opacity: 1,
    ...
    inputEl.addEventListener('input', (e) => {
      ...
      this.moveTextLine();
      if(textEl.value.length === 0 && this.el.components.virtualKeyboard.data.lastPressedKey === 'DEL') {
        this.setLabel();
        this.updateCursorPosition();
      }
    });
    ...
    scene.addEventListener('click-nothing', e => {
      ...
      toggleListeningToKeydown(false)
    });
    ...
    el.addEventListener('click', (event) => {
      ...
      Array.from(allTextboxes).forEach(textbox => {
        if(textbox.getAttribute('id') !== this.el.getAttribute('id') && textbox.querySelector('#keyboard') !== null){
      ...
      // Switching the activation varialbe
      this.data.isactivated = !this.data.isactivated
      this.switchActivated()
      
      toggleListeningToKeydown(this.data.isactivated);
    }); 
  ...
  createCursor() {
    ...
    cursorEl.setAttribute('geometry', {
        primitive: 'plane',
        height: this.el.getAttribute('height') * 0.2 * this.el.getAttribute('sizeCoef') * (2/3),
        width: 0.005  
    });
    ...
    cursorEl.setAttribute('position', {
        x: cursorInitialPositionX, y: 0, z: 0.01 
    });
  ...
  moveCursorRight() {
    ...
    if (textEl.value.length + this.data.moveCursorByAmount >= this.data.numberOfCharactersInTheBox) return
    this.data.moveCursorByAmount += 1
    // If the text has been moved to the left, and now cursor is moving to the right, move the whole textbox
    if (this.data.moveWholeTextInBoxByAmount < 0) this.data.moveWholeTextInBoxByAmount += 1
    this.moveTextLine();
    this.updateCursorPosition();
  },
  ...
  updateCursorPosition() {
    ...
    const numberOfCharactersInTheBox = this.el.getAttribute('text').value.length > this.data.maxCharsAmount ? this.data.maxCharsAmount : this.el.getAttribute('text').value.length
    this.data.numberOfCharactersInTheBox = numberOfCharactersInTheBox // Actual number of chars in the textbox

    // switch here
    let cursorX = cursorInitialPositionX + fontWidth * numberOfCharactersInTheBox + fontWidth * (moveCursorByAmount - this.data.moveWholeTextInBoxByAmount) ;
    
    // Returning if the cursor is at the end of the textbox
    if (cursorX > this.data.textWidthCoef*1.05 * this.el.getAttribute('sizeCoef') / 2) return
    ...
    cursorEl.setAttribute('geometry', {
      primitive: 'plane',
      height: this.el.getAttribute('height') * 0.18 * this.el.getAttribute('sizeCoef') * (2/3),
