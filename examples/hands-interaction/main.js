...
let spawnCount = 0;
const placedAnimals = new Set();
let currentUnplacedAnimal = null;
...
function spawnAnimal() {
  ...
  el.setAttribute("data-animal-id", animal.id);
  ...
  animalEl.addEventListener("grabended", () => {
    setTimeout(() => {
      for (const boxEl of boxEls) {
        ...
        if (placedAnimals.has(animalId)) break;
      }
    }, 100);
  });
...
function checkIfInBox(animalEl, boxEl, animalId) {
  if (placedAnimals.has(animalId)) return;
...
function showSuccessMessage(animalId) {
  ...
  successEl.setAttribute("value", `${animalId.toUpperCase()} placed correctly!`);
...
function showResultsPanel(results) {
  ...
  const scoreColor = correctCount === totalCount ? "rgb(33, 179, 52)" : "rgb(200, 50, 50)";
...
<a-scene auto-xr="autoEnter: true;">
    ...
    <a-entity position="-1 1.5 0.8">
        <a-text value="Water areas" align="center" position="0 0 0" scale="0.4 0.4 0.4" rotation="0 90 0" color="rgb(71, 134, 25)"></a-text>
    </a-entity>

    <a-entity position="1 1.5 0.8">
        <a-text value="Indoor materials" align="center" position="0 0 0" rotation="0 -90 0" scale="0.4 0.4 0.4" color="rgb(71, 134, 25)"></a-text>
    </a-entity>

    <a-entity position="-0.8 1.3 0.3">
        <a-text value="Outdoor materials" align="center" position="0 0 0" rotation="0 90 0" scale="0.4 0.4 0.4" color="rgb(71, 134, 25)"></a-text>
    </a-entity>
    ...
    <a-entity position="-0 1.5 -0.7">
        <a-ar-button id="submitButton" position="-0.8 0 0" size="small" content="Submit" finger-touch opacity="0.5"></a-ar-button>
        <a-ar-button id="addAnimalButton" position="0 0 0" size="small" content="Add animal" finger-touch></a-ar-button>
        <a-ar-button id="helpButton" position="0.8 0 0" size="small" content="Help" primary="rgb(162, 162, 162)" finger-touch></a-ar-button>
    </a-entity>

    <a-entity id="helpPanel" visible="false" position="0 2 -1.5">
        <a-box width="2" height="1.5" depth="0.1" color="#ffffff" opacity="0.9" position="0 0 0"></a-box>
        <a-text value="HOW TO PLAY" align="center" position="0 0.6 0.1" scale="0.2 0.2 0.2" color="black"></a-text>
        <a-text value="1. Click 'Add animal' to spawn an animal and place it in the correct box\n2. To move objects, pinch them with your fingers in the middle of the object\n3. To stretch objects, pinch them with your fingers at the edges of the object\n4. Make sure animals are touching the ground\n5. Submit and get results when done!" align="center" position="0 0.1 0.1" scale="0.12 0.12 0.12" color="black" width="8" wrap-count="30"></a-text>
    </a-entity>

    <a-box data-box-for="fish" position="-0.8 1.2 0.8" depth="0.2" height="0.2" width="0.2" material="color: rgb(120, 231, 241)" opacity="0.5" grabbable stretchable="mode: scale" hands-hoverable></a-box>
    <a-box data-box-for="whale" position="-1 1 0.8" depth="0.7" height="0.4" width="0.4" material="color: rgb(39, 57, 224)" opacity="0.5" grabbable stretchable hands-hoverable></a-box>
    <a-box data-box-for="cat" position="1 0.8 1" depth="0.1" height="0.6" width="0.2" material="src: #carpet" grabbable stretchable="dimensionAxes: x, y" hands-hoverable></a-box>
    <a-box data-box-for="cat" position="1 1.2 1" depth="0.3" height="0.03" width="0.3" material="src: #carpet" grabbable stretchable="dimensionAxes: x, z" hands-hoverable></a-box>
    <a-box data-box-for="dog" position="1 0.8 0.5" depth="0.4" height="0.4" width="0.4" material="src: #textile" grabbable stretchable hands-hoverable></a-box>
    <a-box data-box-for="dog" position="1 0.9 0.7" depth="0.4" height="0.4" width="0.4" material="src: #textile" grabbable stretchable hands-hoverable></a-box>
    <a-box data-box-for="rabbit" position="-0.7 1.1 0.2" depth="0.2" height="0.025" width="0.2" material="src: #grass" grabbable stretchable="dimensionAxes: x, z" hands-hoverable></a-box>
    <a-box data-box-for="pig" position="-0.7 1 0.2" depth="0.3" height="0.025" width="0.3" material="src: #roof" grabbable stretchable="dimensionAxes: x, z" hands-hoverable></a-box>
    <a-box data-box-for="pig" position="-0.7 0.9 0.2" depth="0.3" height="0.025" width="0.3" material="src: #roof" grabbable stretchable="dimensionAxes: x, z" hands-hoverable></a-box>
    <a-box data-box-for="pig" position="-0.7 0.8 0.2" depth="0.3" height="0.05" width="0.3" material="src: #wood" grabbable stretchable="dimensionAxes: x, z" hands-hoverable></a-box>
...
setTimeout(() => {
  ...
  if (helpBtn && helpPanel) {
    helpBtn.addEventListener("button-clicked", () => {
      const isVisible = helpPanel.getAttribute("visible") === "true" || helpPanel.getAttribute("visible") === true;
  ...
  if (submitBtn) {
    submitBtn.addEventListener("button-clicked", () => {
      ...
      const isVisible = resultsPanel && (resultsPanel.getAttribute("visible") === "true" || resultsPanel.getAttribute("visible") === true);
  ...
  if (fish) {
    if (!placedAnimals.has("fish")) {
      currentUnplacedAnimal = {
        el: fish,
        animalId: "fish",
        animal: { hint: "This fish needs a water to swim in with her friends." },
      };
